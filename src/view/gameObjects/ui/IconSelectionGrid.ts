...
export default class IconSelectionGrid extends Phaser.GameObjects.Container {
  ...
  private icons: Map<
    string,
    {
      ...
      outlineGraphic: Phaser.GameObjects.Graphics;
    }
  >;
  ...
  constructor(
    ...
    x: number,
    y: number,
    properties: IconSelectionGridProperties
  ) {
    super(scene, x, y);
    this.properties = {
      ...defaultProperties,
  ...
  private createProfileIconsGrid(): void {
    ...
    profileTextureKeys.forEach((key, index) => {
      ...
      outline.lineStyle(
        this.properties.outlineThickness,
        this.properties.outlineColour
      );
      outline.strokeRect(
  ...
  private getProfileTextureKeys(): string[] {
    ...
    return textureKeys.filter((key) =>
      key.startsWith(this.properties.texturePrefix)
    );
  }
